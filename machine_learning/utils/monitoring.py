...
def create_summary_report(
    ...
    summary_report.run(
        reference_data=reference, current_data=data_sample, column_mapping=col_mapping
    )
    report_filename = os.path.join(
        tmp_folder, f"summary_report_{report_date.strftime('%Y%m%d%H%M%S')}.json"
    )
...
def create_data_drift_report(
    ...
    report_filename = os.path.join(
        tmp_folder, f"data_drift_report_{report_date.strftime('%Y%m%d%H%M%S')}.json"
    )
...
def create_prediction_drift_report(
    ...
    report_filename = os.path.join(
        tmp_folder, f"pred_drift_report_{report_date.strftime('%Y%m%d%H%M%S')}.json"
    )
...
def check_test_failures(test_json: dict):
    ...
    for test in test_json['tests']:
        if test['status'] != 'SUCCESS':
            ...
            elif (test['name'] == 'Drift per Column') and (
                test['parameters']['column_name'] == 'prediction'
            ):
                failure = True
            ...
            # if AvgSessionDurationMinutes is a drifted column
                ...
                test['parameters']['column_name'] == 'AvgSessionDurationMinutes'
            ):
                failure = True
            # if SessionsPerWeek is a drifted column
            elif (test['name'] == 'Drift per Column') and (
                test['parameters']['column_name'] == 'SessionsPerWeek'
            ):
                failure = True
...
def create_reports(report_date, ref_data, curr_data, prediction_col, label_col=None):
    ...
    summary_report = create_summary_report(report_date, ref_data, curr_data, column_mapping)
    # Create the Summary or Data Quality report
    print("Creating Data Drift report")
    data_drift_report = create_data_drift_report(report_date, ref_data, curr_data, column_mapping)

    # Create the Summary or Data Quality report
    print("Creating Prediction Drift report")
    pred_drift_report = create_prediction_drift_report(
        report_date, ref_data, curr_data, column_mapping
    )
